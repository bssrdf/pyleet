...
there is an edge between nodes ui and vi in the original graph, and cnti is the total 
number of new nodes that you will subdivide the edge into. Note that cnti == 0 means you 
...
Given the original graph and maxMoves, return the number of nodes that are reachable from 
node 0 in the new graph.
...
class Solution(object):
    def reachableNodes(self, edges, maxMoves, n):
        ...
        while pq:
        	...
        	res += 1
        	for a in graph[cur]:
        		if move > graph[cur][a] and not visited[a]:
        			heapq.heappush(pq,(-(move - graph[cur][a] - 1), a))
        		graph[a][cur] -= min(move, graph[cur][a])
        		res += min(move, graph[cur][a])
        return res
