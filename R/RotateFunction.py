...
class Solution:
    def maxRotateFunction(self, nums: List[int]) -> int:
        ...
        sm = sum(i*a for i,a in enumerate(A)) 
        ans = sm
        k = n-1
        while k > 0:
            sm += -(n-1)*A[k] + preSum[k] + preSum[n] - preSum[k+1]
            ans = max(ans, sm)
            k -= 1 
        return ans
...
if __name__ == "__main__":
    ...
    print(Solution().maxRotateFunction(nums = [100]))
    print(Solution().maxRotateFunction2(nums = [4,3,2,6]))
    print(Solution().maxRotateFunction2(nums = [100]))
