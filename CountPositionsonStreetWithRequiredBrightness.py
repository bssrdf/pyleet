...
class Solution:
    def meetRequirement(self, n: int, lights: List[List[int]], requirement: List[int]) -> int:
        ...
        for p, r in lights:
            ...
            diff[min(n,p+r+1)] -= 1
        cnt = diff[0]
        if cnt >= requirement[0]: ans = 1
        else: ans = 0  
        # print(diff)
        for i in range(1,n):
            cnt += diff[i]
            # print(i,cnt)
            if cnt >= requirement[i]: ans += 1
        return ans


if __name__ == "__main__":
    print(Solution().meetRequirement(n = 5, lights = [[0,1],[2,1],[3,2]], requirement = [0,2,1,4,1]))
    print(Solution().meetRequirement(n = 1, lights = [[0,1]], requirement = [2]))