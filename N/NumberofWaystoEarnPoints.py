        ...
        for i in range(1, n+1):
            for j in range(T[i-1][0]+1):
                ...
                for k in range(target+1):
                    dp[i][k] = (dp[i][k] + (dp[i-1][k-t] if k >= t else 0)) % MOD
        return dp[n][target]
        ...
        for i in range(1, n+1):
            for k in range(target, -1, -1):
                for j in range(1, T[i-1][0]+1):
                    t = j*T[i-1][1]
                    dp[k] = (dp[k] + (dp[k-t] if k >= t else 0)) % MOD
        return dp[target]
...
if __name__ == '__main__':
    ...
    print(Solution().waysToReachTarget2(target = 6, types = [[6,1],[3,2],[2,3]]))
