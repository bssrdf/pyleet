    ...
    def pathsWithMaxScore(self, board: List[str]) -> List[int]:
        ...
        for i in range(n-1, -1, -1):
            for j in range(n-1, -1, -1):
                ...
                for x, y in [(0,1), (1,0), (1,1)]: # there are at most 3 paths
                    if dp[i][j][0] < dp[i+x][j+y][0]: # always true for the first path (0,1) picked since 
                                                      # dp[i][j][0] initialized with -10**5
                        dp[i][j] = dp[i+x][j+y][:]    # if true for subsequent path, both elements should 
                                                      # come from that path
                    elif dp[i][j][0] == dp[i+x][j+y][0]: # if true, i.e., another path with the same maximum,                                           
                        dp[i][j][1] += dp[i+x][j+y][1]   # add the number of paths onto the current one
                dp[i][j][0] += int(A[i][j]) if i or j else 0 # collect the number if any
