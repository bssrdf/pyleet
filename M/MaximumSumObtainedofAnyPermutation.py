...
class Solution:
    def maxSumRangeQuery(self, nums: List[int], requests: List[List[int]]) -> int:
        ...
        pos = sorted(range(n), key=lambda x: freq[x], reverse=True)        
        perm = [0]*n
        for i in range(n):
            perm[pos[i]] = nums[i]
        preSum = [0]*(n+1)
        for i in range(1,n+1):
            preSum[i] = preSum[i-1] + perm[i-1]
        res = 0
        for i,j in requests:
...
if __name__ == "__main__":
    ...
    print(Solution().maxSumRangeQuery(nums = [1,2,3,4,5,6], requests = [[0,1]]))
    print(Solution().maxSumRangeQuery(nums = [1,2,3,4,5,10], requests = [[0,2],[1,3],[1,1]]))
