...
class Solution:
    def maximumCoins(self, heroes: List[int], monsters: List[int], coins: List[int]) -> List[int]:
        ...
        mx = max(coins)
        ans, preSum = [], [0]*(m+1)
        mc = [(m,c) for m,c in zip(monsters, coins)]
        mc.sort()
        for i in range(1,m+1):
            preSum[i] = preSum[i-1] + mc[i-1][1]
        for h in heroes:
            idx = bisect.bisect_right(mc, (h,mx+1))
...
if __name__ == "__main__":
    print(Solution().maximumCoins(heroes = [1,4,2], monsters = [1,1,5,2,3], coins = [2,3,4,5,6]))
    print(Solution().maximumCoins(heroes = [5], monsters = [2,3,1,2], coins = [10,6,5,2]))
    print(Solution().maximumCoins(heroes = [4,4], monsters = [5,7,8], coins = [1,1,1]))
