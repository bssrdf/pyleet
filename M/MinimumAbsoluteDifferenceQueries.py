...
class Solution:
    def minDifference(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ...
        preSum = [[0]*(n+1) for _ in range(maxI+1)]
        for i,v in enumerate(nums):
            for j in range(1, maxI+1):
               preSum[j][i+1] = preSum[j][i] + (1 if v == j else 0)
        # print(len(preSum), len(preSum[0]))
        # print(preSum)
        ans = []
        for l, r in queries:
            cnt = []
            for j in range(1, maxI+1):
                d = preSum[j][r+1]-preSum[j][l]
                if d > 0:
                    cnt.append(j) 
            ans.append(min([b-a for a,b in zip(cnt, cnt[1:])] or [-1]))
        return ans
    ...
    def minDifference2(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ...
        ans = []
        for l, r in queries: 
            prev, val = 0, float('inf')
            for x in keys: 
                i = bisect.bisect_left(loc[x], l)
...
if __name__ == "__main__":
    ...
    print(Solution().minDifference(nums = [4,5,2,2,7,10], queries = [[2,3],[0,2],[0,5],[3,5]]))
