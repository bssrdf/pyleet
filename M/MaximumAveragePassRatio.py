...
are assigned to. You want to assign each of the extraStudents students to 
a class in a way that maximizes the average pass ratio across all the classes.
...
the extraStudents students. Answers within 10^-5 of the actual answer will 
be accepted.
...
class Solution:
    def maxAverageRatio(self, classes, extraStudents: int) -> float:
        ...
        k = extraStudents
        for p,t in classes:
            # adding the first student to a class makes the biggest change in pass
            # ratio, so use the change as key in pq
            heapq.heappush(pq, (-((p+1)/(t+1)-p/t), p,t))
        while k > 0:
            _, p, t = heapq.heappop(pq)
            p += 1; t += 1 # adding one student to this class
                           #  will result in biggest increase in p/t
            heapq.heappush(pq, (-((p+1)/(t+1)-p/t), p,t))
            k -= 1
        res, n = 0.0, 0
        while pq:
