...
class Solution:
    def mincostTickets(self, days: List[int], costs: List[int]) -> int:
        ...
        for i in range(n-1, -1, -1):
            idx1 = bisect.bisect_left(days, days[i] + 7, lo = i)
            idx2 = bisect.bisect_left(days, days[i] + 30, lo = i)
            dp[i] = min(dp[i+1]+costs[0], dp[idx1]+costs[1], dp[idx2]+costs[2])            
            #print(i, days[i], idx1, idx2, dp[i+1]+costs[0], dp[idx1], dp[idx2], dp[i])
        return dp[0]    
