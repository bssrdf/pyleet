...
arranged in a row and are labeled from 0 to n - 1 from left to right where the ith 
plant is located at x = i. There is a river at x = -1 that you can refill your 
watering can at.
...
class Solution:
    def wateringPlants(self, plants: List[int], capacity: int) -> int:
        ...
        for i in range(len(plants)-1):
            water += plants[i]
            if water == capacity or water + plants[i+1] > capacity:
                ans += i+i+2 
                water = 0
...
if __name__ == "__main__":
    ...
    print(Solution().wateringPlants(plants = [3,2,4,2,1], capacity = 6))
