...
can work to schedule processes each second. As an Amazonian you are tasked 
to find the minimum time required to schedule all the processes in the system 
...
class Solution(object):
    def minimumTimetoSchedule(self, n, ability, processes):
        pq = [-a for a in ability]
        heapq.heapify(pq)
        count = 0
        while processes > 0:
            a = heapq.heappop(pq)
            processes -= -a
            heapq.heappush(pq, round(-a/2))
            count += 1
