...
same order as the given sequence of books.  For example, if we have an ordered list of 
5 books, we might place the first and second book onto the first shelf, the third book 
...
class Solution(object):
    def minHeightShelves(self, books, shelf_width):
        ...
        for i in range(1, n+1):
            width = books[i-1][0]
            height = books[i-1][1]
            dp[i] = dp[i-1] + height # place book i on a new shelf (may not be optimal)
            j = i-1
            while j > 0 and width + books[j-1][0] <= shelf_width:
                height = max(height, books[j-1][1])
                width += books[j-1][0]
                dp[i] = min(dp[i], dp[j-1] + height) # bring books[i-1:j] down and see if it
                                                     # reduces the total heights needed
                j -= 1
        return dp[n]
...
if __name__ == "__main__":
    ...
    print(Solution().minHeightShelves([[1,1],[2,3]], 4))
