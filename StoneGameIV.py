...
should remove only 1 stone and finally Bob removes the last one (7 -> 3 
-> 2 -> 1 -> 0). 
If Alice starts removing 1 stone, Bob will remove 4 stones then Alice 
only can remove 1 stone and finally Bob removes the last one (7 -> 6 -> 
2 -> 1 -> 0).
Example 5:
...
class Solution(object):
    def winnerSquareGame(self, n):
        ...
        if there is any k such that dp[i - k * k] == false,
        it means we can make the other lose the game,
        so we can win the game an dp[i] = true.

        Another thought: if dp[i] is false i.e. one player loses when n is i, 
        the other player must win the game, hence dp[i+k*k] (k=1,2,....) must 
        be True
        '''
        dp = [False]*(n+1)
...
if __name__ == "__main__":
    ...
    print(Solution().winnerSquareGame(47))