...
Find the lexicographically smallest string that starts at a leaf of this tree and ends at 
the root.
    ...
    def smallestFromLeaf(self, root):
        ...
        def helper(node, path):
            if not node.left and not node.right: 
                self.res = min(self.res, chr(node.val+ord('a'))+path)
                return            
            if node.left:  helper(node.left, chr(node.val+ord('a'))+path)             
            if node.right: helper(node.right,chr(node.val+ord('a'))+path)
        helper(root, '')
        return self.res
