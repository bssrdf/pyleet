...
class SORTracker:
    ...
    def add(self, name: str, score: int) -> None:
        loc = PairMin(score, name)
        if self.minh and loc > self.minh[0]:
            pmin = heapq.heappushpop(self.minh, loc)
            heapq.heappush(self.maxh, PairMax(pmin.score, pmin.name))
        else:
            heapq.heappush(self.maxh, PairMax(score, name))

    def get(self) -> str:
        p = heapq.heappop(self.maxh)
        heapq.heappush(self.minh, PairMin(p.score, p.name))        
        return self.minh[0].name
