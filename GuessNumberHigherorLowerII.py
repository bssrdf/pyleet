    ...
    def getMoneyAmount(self, n: int) -> int:
        ...
        def dp(s, e):
            ...
            for  x in range(s, e+1):
                ...
                # because you don't know which way should go, so to guarantee you have 
                # enough money to find the target, you need to prepare the more, 
                # which is max(dp[i][x-1], dp[x+1][j]).
                tmp = x + max(dp(s, x-1), dp(x+1, e))
                res = min(res, tmp) # to minimize the cost
            return res
        return dp(1, n)
