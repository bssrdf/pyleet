    ...
    def furthestBuildingTLE(self, heights, bricks, ladders):
        ...
        def canReach(i):
            ...
            for k in range(1, i+1):
                h = heights[k]-heights[k-1] 
                if h > 0: heapq.heappush(pq, -h)
            while pq:
                if br <= 0: break
                h = heapq.heappop(pq)
                if ld > 0: ld -= 1
                else: br -= (-h)
            return False if pq else True
        left, right = 0, n
        while left < right:
            mid = left + (right-left)//2
            if canReach(mid): left = mid+1
            else: right = mid 
        return left-1
...
if __name__ == '__main__':   
    print(Solution().furthestBuildingTLE([4,2,7,6,9,14,12], 5, 1))
