        ...
        def bfs(i, j, vis):
            ...
            while que:
                ...
                for dx, dy in [(-1, 0), (1,0), (0,1), (0,-1)]:
                    xx, yy = x + dx, y + dy
                    if 0 <= xx < n and 0 <= yy < n and not vis[xx][yy] and grid[xx][yy] != -1:
                        t += grid[xx][yy]
                        vis[xx][yy] = True
                        que.append((xx,yy))
                        # cells.append((xx,yy))
...
if __name__ == "__main__":
    print(Solution().sumRemoteness(grid = [[-1,1,-1],[5,-1,4],[-1,3,-1]]))
    print(Solution().sumRemoteness(grid = [[-1,3,4],[-1,-1,-1],[3,-1,-1]]))