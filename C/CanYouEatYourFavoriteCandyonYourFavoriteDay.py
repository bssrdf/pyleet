...
false otherwise. Note that you can eat different types of candy on the same day, 
...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        ...
        for idx, val in enumerate(queries):
            ...
            # we decrement by 1 since we need at least one candy of type
            mx = presum[candType + 1] - 1
            # if we eat upto capacity each day up to previous candy
...
if __name__ == "__main__":
    print(Solution().canEat(candiesCount = [7,4,5,3,8], queries = [[0,2,2],[4,2,4],[2,13,1000000000]]))