...
class Solution(object):
    def catMouseGame(self, graph):
        ...
        """
        n = len(graph)
        memo = [[[-1 for _ in range(n)] for _ in range(n)] for _ in range(2*n)]
        def dp(steps, x, y):
            if steps == 2*n: return 0 # draw
            ...
            if mouseTurn:
                catWin = True
                for nxtPos in graph[x]:
                    move = dp(steps+1, nxtPos, y)
                    if move == 1:
                        memo[steps][x][y] = 1
                        return memo[steps][x][y]
                    elif move != 2: catWin = False
                if catWin:
                    memo[steps][x][y] = 2
                    return memo[steps][x][y] 
                else: 
                ...
                mouseWin = True
                for nxtPos in graph[y]:
                    if nxtPos == 0: continue
                    move = dp(steps+1, x, nxtPos)
                    if move == 2:
                        memo[steps][x][y] = 2
                        return memo[steps][x][y]
                    elif move != 1: mouseWin = False
                if mouseWin:
                    memo[steps][x][y] = 1
                    return memo[steps][x][y] 
                else: 
