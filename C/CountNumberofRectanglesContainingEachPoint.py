...
points that lie on the edges of a rectangle are also considered to be contained by 
that rectangle.
...
class Solution:
    def countRectangles(self, rectangles: List[List[int]], points: List[List[int]]) -> List[int]:
        ymax = max(r[1] for r in rectangles)
        cordx = [[] for _ in range(ymax+1)]
        ...
        for i, (x,y) in enumerate(points):
            cnt = 0
            for j in range(ymax, y-1, -1):
                idx = bisect.bisect_left(cordx[j], x)                
                cnt += len(cordx[j]) - idx
            ans[i] = cnt
        return ans
