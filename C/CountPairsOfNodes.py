...
integer array edges, the edges in the graph, where edges[i] = [ui, vi] indicates that 
there is an undirected edge between ui and vi. You are also given an integer array queries.

Let incident(a, b) be defined as the number of edges that are connected to either 
node a or b.

The answer to the jth query is the number of pairs of nodes (a, b) that satisfy both 
    ...
    def countPairs(self, n: int, edges: List[List[int]], queries: List[int]) -> List[int]:
        ...
        for q in queries:
            ...
            for (i, j), sh_cnt in shared.items():
                if q < incident[i] + incident[j] and q + sh_cnt >= incident[i] + incident[j]:
                    c -= 1
            ans.append(c)              
        return ans
