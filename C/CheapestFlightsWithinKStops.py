...
class BucketQueue(object):
    def __init__(self, nbuckets):
        self.buckets = [[] for _ in range(nbuckets+1)]
        self.size = nbuckets+1
        self.top = 0
        self.empty = True

    def push(self, p, item):
        self.buckets[p%self.size].append(item)
...
class Solution(object):
    ...
    def findCheapestPriceDijkstraBucketQueue(self, n, flights, src, dst, k):
        '''
        The key difference with the classic Dijkstra algo is, we don't 
        maintain the global optimal distance to each node, i.e. ignore 
        below optimization:
        ...
        for a, b, p in flights:
            f[a][b] = p
            mxp = max(mxp, p)
        heap = BucketQueue(mxp)
        heap.push(0, (src, k+1))
        seen = {}
        while not heap.isEmpty():
            #heap.printQueue()
            cost, (city, stops) = heap.pop()
            #print(city, cost)
            if city == dst:
                return cost
            if city in seen and seen[city] >= stops:
            ...
            if stops > 0:
                for j in f[city]:                    
                    heap.push(cost + f[city][j], (j, stops - 1))
        #for see in seen: print(see, seen[see])
        return -1
...
if __name__ == "__main__":
    ...
    print(Solution().findCheapestPriceDijkstraBucketQueue(7, tickets, 2, 4,1))
    #sys.exit(1)
    ...
    print(Solution().findCheapestPriceDijkstra(7, tickets, 2, 4,1))
    ...
    print(Solution().findCheapestPriceDijkstra(3, [[0,1,100],[1,2,100],[0,2,500]],
                                      0, 2, 1))
    print(Solution().findCheapestPriceDijkstraBucketQueue(3, [[0,1,100],[1,2,100],[0,2,500]],
                                      0, 2, 1))
    ...
    print(Solution().findCheapestPrice(4, [[0,3,59],[2,0,83],[2,3,32],
                                       ...
                                       , 3, 0, 3))
    print(Solution().findCheapestPriceDijkstra(4, [[0,3,59],[2,0,83],[2,3,32],
                                           [0,2,97],[3,1,16],[1,3,16]]
                                       , 3, 0, 3))
    print(Solution().findCheapestPriceDijkstraBucketQueue(4, [[0,3,59],[2,0,83],[2,3,32],
                                           [0,2,97],[3,1,16],[1,3,16]]
                                       , 3, 0, 3))
