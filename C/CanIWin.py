...
class Solution(object):
    def canIWin(self, maxChoosableInteger, desiredTotal):
        ...
        @lru_cache(None) 
        def dfs(picked, acc):
            if acc >= desiredTotal: # we won, the opponent lost
                return False
            if picked == (1 << (maxChoosableInteger+1)) - 1:
                # nothing to choose, we lost
                print(bin(picked))
                return False
            for i in range(1, maxChoosableInteger+1):
                if picked & (1 << i) == 0:
                    # the opponent loses if we pick this i
                    if not dfs(picked | (1 << i), acc+i):
                        return True
            return False
        return dfs(0, 0)
