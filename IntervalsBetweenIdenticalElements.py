...
between their indices. More formally, the interval between arr[i] and arr[j] is |i - j|.
...
class Solution:
    def getDistances(self, arr: List[int]) -> List[int]:
        ...
        for i in range(n):
            j = pos[i]
            pre = preSum[arr[i]]
            # ans[i] = j*i - pre[j+1] + pre[-1] - pre[j] - (len(idx[arr[i]])-j-1)*i     
            ans[i] = (j-len(idx[arr[i]])+j+1)*i - pre[j+1] + pre[-1] - pre[j] 
        return ans
