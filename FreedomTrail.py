...
There are only lowercase letters in both strings and might be some duplcate characters in 
both strings.
...
class Solution(object):
    def findRotateSteps(self, ring, key):
        ...
        for i in range(n): v[ord(ring[i]) - ord('a')].append(i)
        def helper(x, y):
            if y == len(key): return 0
            if memo[x][y] != -1: return memo[x][y]
            res = float('inf')
            # try rotating 12:00 to each pos k where ring[k] == key[y]
            # from there, dfs to next letter of key              
            for k in v[ord(key[y]) - ord('a')]:
                diff = abs(x - k)
                step = min(diff, n - diff) # step contains the smaller one (either clockwise or anticlockwise)                
                res = min(res, step + helper(k, y + 1))
            memo[x][y] = res + 1 # 1 accounts for a button push
            return memo[x][y]
        return helper(0, 0)
