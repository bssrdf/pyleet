...
numbered from 0 to m - 1. The answers of the students are represented by 
a 2D integer array students where students[i] is an integer array that 
...
class Solution:
    def maxCompatibilitySum(self, students: List[List[int]], mentors: List[List[int]]) -> int:
        ...
        def dp(i, mask):
            ...
            for j in range(m):
                if mask & (1 << j) == 0:
                    ans = max(ans, scores[(i,j)]+dp(i+1, mask | (1<<j)))
            return ans        
...
if __name__ == "__main__":
    ...
    print(Solution().maxCompatibilitySum(students = [[0,0],[0,0],[0,0]], mentors = [[1,1],[1,1],[1,1]]))
