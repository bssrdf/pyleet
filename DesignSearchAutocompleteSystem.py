...
only be lower-case letters ('a' to 'z'), blank space (' ') or a special character ('#'). Also, 
...
class Pair(object):
    ...
    def __repr__(self):
        return 'Node value: {self.sentence, self.freq}'
...
class AutocompleteSystem(object):
    ...
    def input(self, c):
        ...
        for k,v in cur.counts.items():
            if len(pq) < 3:
                heapq.heappush(pq, Pair(k,v))
            else:
                heapq.heappushpop(pq, Pair(k,v))
        while pq:
            p = heapq.heappop(pq)
            res.append(p.sentence)
        return res[::-1]

if __name__ == "__main__":
    autosys = AutocompleteSystem(["i love you", "island","ironman", "i love leetcode", 
                          "i love flick"], [5,3,2,2,1])
    print(autosys.input('i'))
    print(autosys.input(' '))
    print(autosys.input('l'))
    print(autosys.input('o'))
    print(autosys.input('v'))
    print(autosys.input('e'))
    print(autosys.input(' '))
    print(autosys.input('f'))
    print(autosys.input('l'))
    print(autosys.input('i'))
    print(autosys.input('c'))
    print(autosys.input('k'))
    print(autosys.input('#'))
    print(autosys.input('i'))
    print(autosys.input(' '))
