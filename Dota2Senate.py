...
from the same party, he can announce the victory and make the decision 
...
his own party, you need to predict which party will finally announce 
the victory and make the change in the Dota2 game. The output should 
be Radiant or Dire.
...
class Solution(object):
    def predictPartyVictory(self, senate):
        ...
        while qr and qd:
            ...
            # in each round the winner will be the one at the front (smalled index)
            # the winning party stays in the queue, but moves to the back
            # the losing party gets out of the queue, never comes back
            if r < d: qr.append(r+n) 
            else: qd.append(d+n) 
        if qr: return "Radiant"
        else: return "Dire"
