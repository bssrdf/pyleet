...
class Solution:
    def maximumsSplicedArray(self, nums1: List[int], nums2: List[int]) -> int:
        def kadane(nums1, nums2):
            # calculate how much nums2 can earn by swaping a part of the array.
            res = cur = 0
            for i in range(len(nums1)):
                # Once the element in nums1 make the cur negative, reset cur to zero; else, update cur.
                cur = max(0, cur + nums1[i] - nums2[i])
                res = max(res, cur)
