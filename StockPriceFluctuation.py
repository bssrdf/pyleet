...
You are given a stream of records about a particular stock. Each record contains 
a timestamp and the corresponding price of the stock at that timestamp.
...
class StockPrice:
    ...
    def update(self, timestamp: int, price: int) -> None:                
        if timestamp in self.minHRecord:
            self.minHRecord[timestamp][1] = True 
        if timestamp in self.maxHRecord:
            self.maxHRecord[timestamp][1] = True 
        self.minHRecord[timestamp] = [price, False]    
        heapq.heappush(self.minHeap, self.minHRecord[timestamp])
        self.maxHRecord[timestamp] = [-price, False]    
        heapq.heappush(self.maxHeap, self.maxHRecord[timestamp])
    ...
    def maximum(self) -> int:
        while self.maxHeap and self.maxHeap[0][1]:
            heapq.heappop(self.maxHeap)
        return -self.maxHeap[0][0]

    def minimum(self) -> int:
        while self.minHeap and self.minHeap[0][1]:
